...
build:
  mjml:
    ...
    enabled: true
    # Directories to search for MJML templates (default: custom plugins paths)
    searchPaths:
      - custom/plugins
      - custom/static-plugins
    # Use web service for compilation instead of local NPM (optional)
    useWebService: false
    # Web service URL (required if useWebService is true)
    webServiceURL: ""
    # API key for web service (required if useWebService is true)
    webServiceAPIKey: ""
```
...
Choose between two compilation methods:

1. **Local compilation** (`useWebService: false`, default): Requires `mjml` package installed via NPM in your build environment
2. **Web service compilation** (`useWebService: true`): Uses an external MJML compilation service (also requires `webServiceURL` and `webServiceAPIKey` to be configured)

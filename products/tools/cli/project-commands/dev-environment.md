...
The `compose.yaml` file is **managed by shopware-cli** and regenerated when you change configuration. Create a `compose.override.yaml` file to customize services without affecting the managed configuration.
...
The generated `compose.yaml` is managed by shopware-cli. Customize it via `compose.override.yaml` rather than editing it directly:
...
The `compose.yaml` file is managed by shopware-cli and regenerated when configuration changes. Use `compose.override.yaml` for persistent customizations.

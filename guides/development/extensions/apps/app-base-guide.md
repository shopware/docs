...
Set up [Continuous Integration (CI)](../testing/ci.md) early. Validate your manifest, test registration and signature handling, verify webhook handling, and build reproducible release artifacts to prevent upgrade regressions and breaking changes in communication.

:::info
Even though apps run externally, version compatibility still matters. Test against target Shopware versions early and keep webhook handling tolerant to new fields or headers.
:::

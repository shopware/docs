...
However, if we had to consider all classes and properties as public API, we would be very limited in our work.

Therefore, we mark the elements that we do not consider to be the public API. To do this, we have the following tools at our disposal.

## Decoration pattern

Classes that are intended for **service decoration** are provided with an abstract class. This class is then provided with a `getDecorated` function to pass unimplemented functions directly to the core classes. Refer to [decoration pattern](https://github.com/shopware/shopware/blob/trunk/adr/2020-11-25-decoration-pattern.md) for more details.
...
- We declare **DTO classes** as `final` to indicate that we do not intend third-party developers to derive from these classes. To append more data to DTO classes, we use the base `Struct` class, which allows **Extensions**.
- We declare **Event Subscriber** as `final` as we do not foresee deriving from them to leverage the events or extend their functionality.

Classes we declare as `final` are still part of the Public API because **Third-Party Developers are consumers** of these classes. That means they access the public methods and functions of the classes.

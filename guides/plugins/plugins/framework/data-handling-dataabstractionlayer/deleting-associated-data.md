...
Let's start with the normal one. Usually, a `OneToMany` association is just the other side of a `ManyToOne` association, whose deletion was already explained in the section about \[PLACEHOLDER-LINK: deleting ToOne associations\].
...
There's been a lot of talking about associations in entities. Maybe it's time now to add your own entities via a plugin \[PLACEHOLDER-LINK: Add custom complex data\] and to add associations to it then\[PLACEHOLDER-LINK: Add data associations\].

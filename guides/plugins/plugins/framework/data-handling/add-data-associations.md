...
{% hint style="warning" %}
Setting autoload to `true` on the `EntityExtension` and `EntityDefinition` will lead to a recursion / out of memory error. If you want to get the association on every load set autoload to `true` only in the `EntityExtension`. See also [Add complex data to existing entities](../add-complex-data-to-existing-entities#adding-a-field-with-database).
{% endhint %}

...
In this guide you will learn how to properly fetch data from the database in your plugin or for core contributions.
...
It's worth getting used to the "DAL", as you might stumble upon this term now and then in the Shopware universe.
...
Furthermore, the guide about [Dependency injection](../../services/dependency-injection.md) will come in handy, since you need to know how to inject a service using the DI container.
You also might want to have a look at the concept behind the [Data abstraction layer](../../../../../concepts/framework/data-abstraction-layer.md) to get a better grasp of how it works.
...
use Swag\BasicExample\Service\ReadingData;
...
If you want to fetch data for another entity, switch the `id` in the `services.php` to whatever repository you need, e.g. `order.repository` for orders.

### Using the repository

Now that you've injected the repository into your service, you can start using it.
First, for the following examples you'll need two more imports:
...
That's it already.
It will read some products without any special filtering.
...
The `Criteria` object accepts an array of IDs to search for as a constructor parameter.
This means that you could apply more than just one ID here.

The `search` method will then return a `EntitySearchResult`, which contains the according entity collection of all products.
Even though just one product can be matched here, the method will always return a collection, which then contains your single product.
Therefore, we're calling `first()` to get the actual entity, and not the collection as a return.
...
To do this, you can apply filters to the `Criteria` object, such as an `EqualsFilter`, which accepts a field name and the value to search for.
...
Since the `EntitySearchResult` is extending the `EntityCollection`, which is iterable, you could iterate over the results using a `foreach`.
...
E.g.: Fetch all products, whose name is `Example product`, but also return the total number of products available.

In that case, you can use the `addPostFilter` instead of `addFilter`:
...
You can find all other filters either on [GitHub](https://github.com/shopware/shopware/tree/trunk/src/Core/Framework/DataAbstractionLayer/Search/Filter) or in our [filters reference](../../../../development/troubleshooting/dal-reference/filters-reference.md) with explanation.

#### Associations

Of course, associations to other entities are also possible in Shopware 6.
If you, for example, want to load all product-reviews, which are entities themselves, related to the product you have found,
you can do so by adding associations to the criteria object.
...
Also, worth mentioning is the fact that you can chain the association key.
E.g. a product-review has another association to the customer who created that review.
If you want access to both the review itself, and the customer, you can write the association like that:
...
Yes, this is doable.
You can apply filters to an association.
E.g. "Add all product reviews to the product, whose rating is above four stars".
...
Once again: Note that we used `getAssociation` here now instead of `addAssociation`.
Also, you need the `RangeFilter`, which can be found here: `Shopware\Core\Framework\DataAbstractionLayer\Search\Filter\RangeFilter`
...
Of course, you can also aggregate your data.
Just like filters and associations, this can be done by using an `addAggregation` method on the `Criteria` object.
...
Important to note here is that you have to remove the `first()` call,
because we do **not** need the entity itself but the `EntitySearchResult` here instead.
...
There are just a few more things missing:
...
That's quite self-explanatory, isn't it?
Use the `setLimit` method with your desired limit as parameter.
...
This way you get the second possible product.
But since you didn't define a sorting yourself, the result can be quite confusing, so let's add a sorting.
...
Exact result counts can make large, filtered listings expensive.
Select the cheapest total count mode that your UI needs:

* `Criteria::TOTAL_COUNT_MODE_NONE` skips the count when no pagination is needed (default).
* `Criteria::TOTAL_COUNT_MODE_EXACT` returns the exact total for pagination with a "last page".
* `Criteria::TOTAL_COUNT_MODE_NEXT_PAGES` uses a bounded look-ahead and suits next-page links or infinite scrolling.
...
But what does it do?
Basically, it's a little helper class that helps you deal with big data sets by being iterable and returning a batch of data with each iteration, but never all data at once.

Imagine you need to iterate over all products of your shop, which contains more than 100,000 products.
...
Just be sure to not use it unnecessarily, since it will create a new database request with each iteration, which is not needed for smaller chunks of data.
...
For entities with an auto-increment field, an iterator without explicit sorting automatically uses that field for key-set pagination.
This keeps deep iterations efficient with both `fetch()` and `fetchIds()`; prefer `fetchIds()` when you only need IDs.

One small limitation to be aware of: When adding explicit sorting to a `RepositoryIterator`, make sure that the sorting is deterministic.
Explicit sorting also disables the automatic auto-increment optimization.

Put differently, you must ensure that your sorting means that there's only one correct way to order your results.
Otherwise, different batches might decide to sort the entities differently in the database.
That would mean you risk getting the same entity in several batches (and having entities that won't be iterated at all).

For example, ordering products by `manufacturerNumber` alone could cause this issue, because several products can have the same `manufacturerNumber`, so there are several correct orderings of those products.
On the other hand, because each product is guaranteed to have a unique ID, sorting by ID is an easy way to mitigate this issue:

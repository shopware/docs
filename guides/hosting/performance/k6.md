...
K6 is a modern load testing tool that makes it easy to test the performance of your Shopware store. It runs scenearios defined in JavaScript and can be used to simulate hunders of users accessing your store at the same time, so you can see how your store performs under load.
...
First we need to clone the [Shopware K6 repository](https://github.com/shopware/k6-shopware) and install the dependencies:
...
and copy `.env.example` to `.env` and adjust the values to your Shopware store:

```bash
cp .env.example .env
```

after setting up the credentials we need to fetch the fixtures (salutation IDs, country IDs, sales channel configuration):
...
## Prepariations on Shopware Side
...
The command `framework:demodata` can be also execute multiple times parallel, so you can create a lot of products in a short time. Just make sure that you run `dal:refresh:index` after all processes are finished.
...
This allows you to ultiize the K6 Cloud infrastructure to run the tests with a lot of more users, customize the location of the users and get more detailed reports with Grafana Dashboards.
